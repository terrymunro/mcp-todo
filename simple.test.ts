...
describe("Database Schema", () => {
  ...
  test("should create and query todo lists", async () => {
    // Insert a todo list
    const result = await testDb.insert(todoList).values({
      name: "Test List",
    ...
    // Query it back
    const retrieved = await testDb.select().from(todoList).where(eq(todoList.id, result[0].id));
  ...
  test("should create and query projects", async () => {
    ...
    const todoListResult = await testDb.insert(todoList).values({
      name: "Default List",
      description: "Default todo list",
    }).returning();
    
    // Then create a project
    const projectResult = await testDb.insert(project).values({
      name: "Test Project",
      location: "/test/path",
      default_todo_list_id: todoListResult[0].id,
    }).returning();
    ...
    expect(projectResult[0].default_todo_list_id).toBe(todoListResult[0].id);
    
    // Query it back
    const retrieved = await testDb.select().from(project).where(eq(project.id, projectResult[0].id));
    expect(retrieved.length).toBe(1);
  ...
  test("should create and query todos", async () => {
    ...
    // Then create a todo
    const todoResult = await testDb.insert(todo).values({
    ...
    // Query it back
    const retrieved = await testDb.select().from(todo).where(eq(todo.id, todoResult[0].id));
    expect(retrieved.length).toBe(1);
  ...
  test("should sort todos by priority correctly", async () => {
    ...
    const todos = await testDb.select().from(todo).where(eq(todo.todo_list_id, listId));
  ...
  test("should cascade delete todos when todo list is deleted", async () => {
    ...
    // Verify todos exist
    const todosBefore = await testDb.select().from(todo).where(eq(todo.todo_list_id, listId));
    ...
    const todosAfter = await testDb.select().from(todo).where(eq(todo.todo_list_id, listId));
...
describe("Data Directory Path Resolution", () => {
  test("should handle XDG_DATA_HOME environment variable", () => {
    ...
    try {
      ...
      const expectedPath = xdgDataHome ? `${xdgDataHome}/mcp-todo` : `${process.env.HOME}/.local/share/mcp-todo`;
